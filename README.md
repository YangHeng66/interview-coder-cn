...
- 可选开启语音自动回答：识别到句末或短暂停顿后自动提交；新的语音问题会中止当前回答并按顺序继续处理
...
以下截图采自本地应用，问题、回答和岗位信息均为演示内容。

### 截图解题：题目与解答同屏展示

截图缩略图与解答集中展示，支持 Markdown 排版和代码高亮。可以追加截图补充题目信息，也可以通过追问继续讨论解题思路。

![截图解题界面：题目缩略图、TypeScript 示例解答和追问入口](./docs/images/01-screenshot-analysis.png)

### 文字对话：独立的问答工作区

文字对话拥有独立的模型配置和会话上下文，支持直接输入问题、附加文本文件，以及发送语音转录内容。下图以 JavaScript 事件循环为例，展示问答与代码块的阅读效果。

![文字对话界面：JavaScript 事件循环示例问答与消息输入区](./docs/images/02-text-chat.png)

### 岗位知识库：岗位资料与内置知识结合

按岗位管理 JD、简历、项目说明和面试笔记，为回答提供相关参考。内置前端知识包覆盖五个主题，可在页面中预览资料并控制是否参与检索。

![岗位知识库界面：示例岗位档案与五类内置前端知识](./docs/images/03-knowledge-base.png)

### 语音转录：配置音频来源与自动回答

可选择阿里云百炼或火山引擎豆包，支持系统音频、麦克风和混合音频。开启语音自动回答后，识别到句末或停顿时会自动提交到文字对话。

![语音转录设置：音频来源、自动回答开关、服务商和音频设备](./docs/images/04-transcription-settings.png)

### 悬浮工具条：集中操作桌面助手

通过独立工具条执行截图、追加截图、停止生成、鼠标穿透、翻页、移动窗口、调整透明度和语音操作；也可在设置中配置悬停触发。

![悬浮工具条：截图、停止、鼠标穿透、窗口调整与语音操作按钮](./docs/images/05-overlay-toolbar.png)
